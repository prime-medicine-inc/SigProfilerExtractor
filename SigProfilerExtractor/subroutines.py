...
def normalize_samples(bootstrapGenomes, totalMutations, norm="100X", normalization_cutoff=10000000):
    ...
    if norm == "gmm":
        ...
        norm_genome = bootstrapGenomes[:, list(indices)] / totalMutations[list(indices)][:,
                                                           np.newaxis].T * normalization_cutoff
        ...
        try:
            ...
            norm_genome = bootstrapGenomes[:, list(indices)] / totalMutations[list(indices)][:, np.newaxis].T * (
                int(norm))
            bootstrapGenomes[:, list(indices)] = norm_genome
            bootstrapGenomes = pd.DataFrame(bootstrapGenomes)
        except:
...
def nnmf_cpu(genomes, nfactors, init="nndsvd", execution_parameters=None, generator=None):
    ...
    net = nmf_cpu.NMF(genomes, rank=nfactors, min_iterations=min_iterations, max_iterations=max_iterations,
                      tolerance=tolerance, test_conv=test_conv, init_method=init, generator=generator,
...
def BootstrapCancerGenomes(genomes, seed=None):
    ...
    repmat = pd.concat([a] * genomes.shape[
        0])  # normalize the data to get the probility of each datapoint to perform the "random.multinomial" operation
...
def pnmf(batch_generator_pair=[1, None], genomes=1, totalProcesses=1, resample=True, init="nndsvd",
         normalization_cutoff=10000000, norm="log2", gpu=False, execution_parameters=None):
    tic = time.time()
    ...
    if gpu:
        ...
        for b in range(batch_size):
            ...
            bootstrapGenomes = normalize_samples(bootstrapGenomes, totalMutations, norm=norm,
        ...
        W, H, kl = nmf_fn(bootstrapGenomes, totalProcesses, init=init, execution_parameters=execution_parameters,
                          generator=rand_rng)  # uses custom function nnmf
...
def decipher_signatures(execution_parameters, genomes=[0], i=1, totalIterations=1, cpu=-1, mut_context="96",
    ...
    print("Extracting signature {} for mutation type {}".format(totalProcesses,
                                                                mut_context))  # m is for the mutation context
    ...
    if gpu == True:
        ...
        results = parallel_runs(execution_parameters, genomes=genomes, totalProcesses=totalProcesses, \
                                verbose=False, replicate_generators=replicate_generators)
    toc = time.time()
    print("Time taken to collect {} iterations for {} signatures is {} seconds".format(totalIterations, totalProcesses,
                                                                                       round(toc - tic, 2)))
    ##############################################################################################################################################################################       
...
################################################################### FUNCTION  ###################################################################
def pairwise_cluster_raw(mat1=([0]), mat2=([0]),
                         dist="cosine"):  # the matrices (mat1 and mat2) are used to calculate the clusters and the lsts will be used to store the members of clusters
...
def cluster_converge_innerloop(Wall, Hall, totalprocess, iteration_generator_pair, iteration=1, dist="cosine",
                               gpu=False):
    ...
    convergence_count = 0
    while True:
        processAvg, exposureAvg, processSTE, exposureSTE, avgSilhouetteCoefficients, clusterSilhouetteCoefficients = reclustering(
            Wall, Hall, processAvg, exposureAvg, dist=dist, gpu=gpu)
...
def cluster_converge_outerloop(Wall, Hall, totalprocess, dist="cosine",
    ...
    for i in range(50):  # using 10 iterations to get the best clustering 
        ...
        temp_processAvg, temp_exposureAvg, temp_processSTE, temp_exposureSTE, temp_avgSilhouetteCoefficients, temp_clusterSilhouetteCoefficients = \
        result_list[i][0], result_list[i][1], result_list[i][2], result_list[i][3], result_list[i][4], result_list[i][5]
...
def read_csv(filename, folder=False):
    ...
    if mtypes == ['96']:
        ...
        orderlist = ['A[C>A]A', 'A[C>A]C', 'A[C>A]G', 'A[C>A]T', 'A[C>G]A', 'A[C>G]C', 'A[C>G]G', 'A[C>G]T', 'A[C>T]A',
                     'A[C>T]C', 'A[C>T]G', 'A[C>T]T',
                     'A[T>A]A', 'A[T>A]C', 'A[T>A]G', 'A[T>A]T', 'A[T>C]A', 'A[T>C]C', 'A[T>C]G', 'A[T>C]T', 'A[T>G]A',
                     'A[T>G]C', 'A[T>G]G', 'A[T>G]T',
                     'C[C>A]A', 'C[C>A]C', 'C[C>A]G', 'C[C>A]T', 'C[C>G]A', 'C[C>G]C', 'C[C>G]G', 'C[C>G]T', 'C[C>T]A',
                     'C[C>T]C', 'C[C>T]G', 'C[C>T]T',
                     'C[T>A]A', 'C[T>A]C', 'C[T>A]G', 'C[T>A]T', 'C[T>C]A', 'C[T>C]C', 'C[T>C]G', 'C[T>C]T', 'C[T>G]A',
                     'C[T>G]C', 'C[T>G]G', 'C[T>G]T',
                     'G[C>A]A', 'G[C>A]C', 'G[C>A]G', 'G[C>A]T', 'G[C>G]A', 'G[C>G]C', 'G[C>G]G', 'G[C>G]T', 'G[C>T]A',
                     'G[C>T]C', 'G[C>T]G', 'G[C>T]T',
                     'G[T>A]A', 'G[T>A]C', 'G[T>A]G', 'G[T>A]T', 'G[T>C]A', 'G[T>C]C', 'G[T>C]G', 'G[T>C]T', 'G[T>G]A',
                     'G[T>G]C', 'G[T>G]G', 'G[T>G]T',
                     'T[C>A]A', 'T[C>A]C', 'T[C>A]G', 'T[C>A]T', 'T[C>G]A', 'T[C>G]C', 'T[C>G]G', 'T[C>G]T', 'T[C>T]A',
                     'T[C>T]C', 'T[C>T]G', 'T[C>T]T',
                     'T[T>A]A', 'T[T>A]C', 'T[T>A]G', 'T[T>A]T', 'T[T>C]A', 'T[T>C]C', 'T[T>C]G', 'T[T>C]T', 'T[T>G]A',
...
def export_information(loopResults, mutation_type, output, index, colnames, sequence="genome", wall=False):
    ...
    exposures.to_csv(activity_subdirectory + "/" + mutation_type + "_S" + str(i) + "_NMF_Activities.txt", "\t",
                     index_label=[exposures.columns.name])

    # plt tmb
    tmb_exposures = pd.melt(exposures)
    tmb.plotTMB(tmb_exposures, scale=sequence, Yrange="adapt",
                output=activity_subdirectory + "/" + mutation_type + "_S" + str(i) + "_" + "TMB_NMF_plot.pdf")
    del tmb_exposures

    # plot activities
    plot_ac.plotActivity(activity_subdirectory + "/" + mutation_type + "_S" + str(i) + "_NMF_Activities.txt",
                         output_file=activity_subdirectory + "/" + mutation_type + "_S" + str(
                             i) + "_" + "NMF_Activity_Plots.pdf", bin_size=50, log=False)
    ...
    processSTE.to_csv(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures_SEM_Error" + ".txt",
                      "\t", float_format='%.2E', index_label=[processes.columns.name])
    ...
    exposureSTE.to_csv(activity_subdirectory + "/" + mutation_type + "_S" + str(i) + "_NMF_Activities_SEM_Error.txt",
                       "\t", float_format='%.2E', index_label=[exposures.columns.name])
    ...
    signature_stats.to_csv(stats_subdirectory + "/" + mutation_type + "_S" + str(i) + "_" + "Signatures_stats.txt",
    ...
    converge_information.to_csv(
        stats_subdirectory + "/" + mutation_type + "_S" + str(i) + "_" + "NMF_Convergence_Information.txt", "\t",
    ...
    print(
        'The reconstruction error is {}, average process stability is {} and \nthe minimum process stability is {} for {} signatures\n\n'.format(
            reconstruction_error, round(meanProcessStability, 2), round(minProcessStability, 2), i))
    fh.write('{}, {}, {}, {}\n'.format(i, round(minProcessStability, 2), round(reconstruction_error, 4),
    ...
    if m == "DBS78":
        plot.plotDBS(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                     signature_subdirectory + "/Signature_plot/", "S" + str(i), "78", True,
                     custom_text_upper=stability_list, custom_text_middle=total_mutation_list)
    elif m == "ID83":
        plot.plotID(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                    signature_subdirectory + "/Signature_plot/", "S" + str(i), "83", True,
                    custom_text_upper=stability_list, custom_text_middle=total_mutation_list)
    elif m == "CNV48":
        plot.plotCNV(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                     signature_subdirectory + "/Signature_plot/", "S" + str(i), "pdf", percentage=True, aggregate=False)
    elif m == "SV32":
        plot.plotSV(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                    signature_subdirectory + "/Signature_plot/", "S" + str(i), "pdf", percentage=True, aggregate=False)
        ...
        if m == "SBS96" or m == "SBS288" or m == "SBS384" or m == "SBS1536":
            plot.plotSBS(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                         signature_subdirectory + "/Signature_plot/", "S" + str(i), tmp_m, True,
                         custom_text_upper=stability_list, custom_text_middle=total_mutation_list)
        elif m == "SBS4608":
            plot.plotSBS(signature_subdirectory + "/" + mutation_type + "_S" + str(i) + "_Signatures" + ".txt",
                         signature_subdirectory + "/Signature_plot/", "S" + str(i), tmp_m, True)
...
def stabVsRError(csvfile, output, title, all_similarities_list, input_type="csvfile", stability=0.8, min_stability=0.2,
                 combined_stability=1.0, mtype="", statistics=True, select=None, sequence="genome",
    ...
    maximum_kl = [];
    mean_correlation = [];
    minimum_correlation = [];  # all_mean_l2=[];
    ...
    selection_data = selection_data_to_sort.sort_values(by=['avgStability', 'Total Signatures'],
    ...
    if len(list_of_idx_over_thresh_hold) > 1 and statistics == True:
        while True:
            ...
            signatures_within_thresh_hold = signatures_within_thresh_hold - (
                        list_of_idx_over_thresh_hold[strating_idx] - list_of_idx_over_thresh_hold[
                    strating_idx - 1])  # get the difference between current and previous stable signatures
            all_similarities = all_similarities_list[idx_within_thresh_hold].iloc[:, [1, 3, 5, 6, 7]]
            ...
            if (wiltest < wiltest_thr) and (current_mean - init_mean > 0) or idx_within_thresh_hold == 0:
                final_solution = signatures_within_thresh_hold + (
                            list_of_idx_over_thresh_hold[strating_idx] - list_of_idx_over_thresh_hold[
                        strating_idx - 1])  # select the previous stable signatures
                break
